...
def next_receipt_no(cursor, tenant_id: int, prefix: str) -> str:
    ...
    try:
        ...
        result = cursor.fetchone()
        if not result:
            raise Exception("Failed to get sequence number from ReceiptSequence table")
            
        seq_number = str(result['last_no']).zfill(6)
        
        return f"{prefix}-{yyyymm}-{seq_number}"
    except Exception as e:
        print(f"ERROR in next_receipt_no: {str(e)}")
        raise Exception(f"Failed to generate receipt number: {str(e)}")
